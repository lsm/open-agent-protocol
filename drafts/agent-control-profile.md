...
| Tool-source attachment control | `action.tool_sources.attach` (executable, `+tool-sources`: `modes` discloses `session_open` and, where offered, `remote`; `limits` discloses `max_sources` and `transports`; fixtures `open-attach-process-source`, `open-attach-unadvertised`, `tool-source-attach-refused-within-limits`) |
