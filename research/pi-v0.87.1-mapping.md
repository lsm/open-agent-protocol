# Pi coding agent v0.87.1 mapping ledger

Status: implemented production OAP adapter, re-pinned from v0.85.1
(`pi-v0.85.1-mapping.md`, now retired) to v0.87.1. Everything below is carried
from that ledger except where "Changes from v0.85.1" says otherwise.
...
- Release: `v0.87.1` (published 2026-09-22)
- Commit: `f07218c4d4bbc12bef056a7058c3dd49dfe41abe`
- Commit tree: `7053e72909a45c2e400f0506c4aa98e53b26f429`

Reproduce the pin:

```sh
git clone https://github.com/earendil-works/pi.git
git -C pi checkout f07218c4d4bbc12bef056a7058c3dd49dfe41abe
git -C pi rev-parse HEAD 'HEAD^{tree}'   # f07218c..., 7053e72...
git -C pi describe --tags                # v0.87.1
```

Inspected source blobs at this pin (`git rev-parse v0.87.1:<path>`):

| Source | v0.85.1 blob | v0.87.1 blob |
|---|---|---|
| `modes/rpc/rpc-types.ts` | `1cbd49a8` | `1cbd49a8` (unchanged) |
| `modes/rpc/rpc-mode.ts` | `fc8083be` | `f4857ffb` |
| `core/agent-session.ts` | `ac2bd4b1` | `338d0e0d` |
| `core/session-manager.ts` | `d25b196a` | `fd87a2d8` |
| `packages/agent/src/types.ts` | `eebb5dc0` | `8da843f5` |
| `rpc-entry.ts` | `11059a8d` | `11059a8d` (unchanged) |
| `cli/args.ts` | `8ad5da63` | `a3587980` |

Full blob ids are in `fixtures/adapters/pi-v0.87.1/manifest.json`.

## Changes from v0.85.1

Found by diffing the seven sources above between the two tags and by running
the same RPC command scripts against both released `pi-darwin-arm64` binaries
behind the loopback Responses mock, comparing every frame's JSON shape.

Wire changes:

1. **`system` role messages (new).** `@earendil-works/pi-ai` adds
   `SystemMessage { role: "system", content: string | TextContent[],
   sections?: Record<string, string | null>, toolsAdded?: Tool[],
   toolsRemoved?: ToolReference[], timestamp }` to `Message`, and the agent
   loop now carries the system prompt and tool declarations as transcript
   messages. On the wire, the first run of a session emits
   `message_start`/`message_end` for a `system` message after `turn_start`
   and before the user message, and that message leads `agent_end.messages`,
   `get_messages` and the persisted entries. Later runs in the same session
   emit none unless the prompt or the tool set changes. Both adapters
   rejected an unknown role, so v0.85.1 code failed every first run against
   v0.87.1 with `pi_invalid_message_end` (the Go integration gate reported
   `run.failed`). Both now accept a closed `system` shape and project nothing
   from it: it is harness transcript state with no OAP core meaning, and it is
   never a final-message candidate.
2. **`entry_appended` on auto-retry.** An auto-retry now appends a
   `context_edit` session entry and emits `entry_appended` for it after
   `auto_retry_start`. The kind and its closed shape are unchanged, and it
   stays observed-only, so no code change.
3. **`ToolResultMessage.addedToolNames` removed** from the upstream type. The
   probe never observed it; both decoders still admit it as an optional
   member, which is harmless to a message the reducer does not project.

Not wire changes: `rpc-types.ts` and `rpc-entry.ts` are byte-identical;
`rpc-mode.ts` only passes `{ source: "rpc" }` into `steer`/`followUp`;
`args.ts` now reports a bad `--mode` value as a diagnostic, and the adapter
always passes `rpc`; the `AgentSessionEvent` union is identical. `get_state`,
`prompt`, `steer`, `follow_up`, `clear_queue`, `abort`, `compact`,
`get_fork_messages`, `new_session`, `get_entries`, `get_commands`,
`get_tree`, tool execution, streaming `message_update` and retry frames have
the same shapes at both tags.

The advertised surface is unchanged. The revisions move to
`pi-v0.87.1-oap-v1` and `pi-v0.87.1-oapx-v1` with the pin.
...
`fixtures/adapters/pi-v0.87.1/` contains eleven compact cases, each with exactly
...
- The other ten cases are **carried forward unchanged** from
  ...
  (no `system` message, no retry `entry_appended`), as the probe comparison
  above shows. Only their `case.json` and `manifest.json` provenance moved to
  this pin, and their `expected-oap.json` differs only in the revision.

Separately gated real-process tests use a caller-supplied Pi v0.87.1 executable:
...
available; v0.85.1 was gated on `pi-linux-x64`). The v0.85.1
`pi-darwin-arm64.tar.gz` used for the shape comparison has digest
`d5f70e3c0cf7398eac239fd0261ee074d98b7ba7f6b43fe3617f052ed5b79d06`.

- `OAP_PI_SMOKE=1`: **PASS**.
- `OAP_PI_INTEGRATION=1`: **FAIL** before the fix (`run.failed` on the
  `system` message), **PASS** after, 3x back to back.
- `oapx serve agent --backend pi` driven over stdio against the same binary
  ...
  text. `goap conformance` against it fails the model-switch checks
  identically with the v0.85.1 binary: the port does not advertise
  `session.model.switch` or `run.model_selection` at either pin.
...
- It advertises revision `pi-v0.87.1-oapx-v1`, with `run.resume` and
