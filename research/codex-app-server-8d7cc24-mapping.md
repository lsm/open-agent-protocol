...
| Session memory | Once a settled session has grown 256 KiB past its last compaction, its arena is rebuilt from what later runs consult: the thread, the session state and each run's id, turn, status and sequence | garbage-collected | Bounded process memory. |
