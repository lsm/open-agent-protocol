...
- `corpus_test.go` (`Test<Harness>EvidenceCorpus`): the hermetic corpus. Reads `fixtures/adapters/<pin>/<case>/` (`native.jsonl` → production reducer → compared with `expected-oap.json`, with `omissions.json` naming each deliberately unmapped native frame and why, and `mapping.json` classifying each frame's fidelity). The corpus manifest pins the upstream commit, tree, and source blob hashes. Seven corpora also decode each native frame through the adapter's production codec (`rpc.ParseMessage`, `rpc.NewDecoder`, `stdio.NewDecoder`, or `native.Decode*`), so framing regressions surface there. The Codex corpus is the exception: its frames are a pre-classified wrapper unmarshaled with `encoding/json` and injected through the fake client, which builds inbound messages directly, so Codex framing and message parsing are covered only by the `internal/rpc` package tests.
