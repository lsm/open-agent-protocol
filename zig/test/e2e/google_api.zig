...
test "google e2e: generative ai (cheap model)" {
    const key = envOwned(testing.allocator, "GOOGLE_API_KEY") orelse {
        std.debug.print("\n\x1b[90mSKIPPED\x1b[0m: google e2e requires GOOGLE_API_KEY\n", .{});
        return error.SkipZigTest;
    };
    defer testing.allocator.free(key);

    const model_id = envOwned(testing.allocator, "GOOGLE_MODEL") orelse try testing.allocator.dupe(u8, "gemini-2.5-flash");
