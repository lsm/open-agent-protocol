...
const codex = @import("codex_adapter");
...
const revision = codex.capability_revision;
const session_id = "e2e";

const Conversation = struct {
    fake: codex.FakeCodex,
    ...
    fn init(self: *Conversation, script: []const u8) !void {
        self.fake = try codex.FakeCodex.init(testing.allocator, script);
        self.adapter = codex.Adapter.init(testing.allocator, self.fake.config());
    ...
    fn approve(self: *Conversation, gate: std.json.Value, choice: []const u8) ![]std.json.Value {
        ...
        const payload = try std.fmt.allocPrint(scratch,
            \\{{"interaction_id":"{s}","requested_by":"{s}","responded_by":"{s}","session_id":"{s}","run_id":"{s}","granted":true,"choice_id":"{s}"}}
        , .{ asked.get("interaction_id").?.string, asked.get("requested_by").?.string, asked.get("responded_by").?.string, session_id, run_id, choice });
        const scope = try std.fmt.allocPrint(scratch, ",\"session_id\":\"{s}\",\"run_id\":\"{s}\",\"capability_revision\":\"{s}\"", .{ session_id, run_id, revision });
        return self.send(try self.envelope("action.permission.resolve.request", scope, payload));
    }
...
test "an approved codex turn served through the endpoint completes, and the whole exchange validates" {
    var conversation: Conversation = undefined;
    try conversation.init(codex.fake_prelude ++ codex.fake_approval_turn ++ codex.fake_idle);
    ...
    const gate = try conversation.pumpUntil("action.permission.requested");
    try testing.expectEqualStrings("conformance", gate.object.get("payload").?.object.get("responded_by").?.string);
    const resolved = try conversation.approve(gate, "accept");
    try testing.expectEqualStrings("action.permission.resolve.response", typeOf(resolved[0]));
    ...
    const stale = try conversation.probe(try conversation.envelope("session.state.request", ",\"session_id\":\"" ++ session_id ++ "\",\"capability_revision\":\"codex-0-stale\"", "{\"session_id\":\"" ++ session_id ++ "\"}"));
    ...
    try testing.expect(std.mem.indexOf(u8, written, "{\"id\":7,\"result\":{\"decision\":\"accept\"}}") != null);
}

test "a codex run cancelled through the endpoint is acknowledged before it settles cancelled, and the exchange validates" {
    var conversation: Conversation = undefined;
    try conversation.init(codex.fake_prelude ++ codex.fake_interrupted_turn ++ codex.fake_idle);
    defer conversation.deinit();
    try conversation.open();
...
test "a codex child that exits mid-run fails the run it held, and the exchange validates" {
    var conversation: Conversation = undefined;
    try conversation.init(codex.fake_prelude ++
