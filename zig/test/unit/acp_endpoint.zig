...
const revision = acp.capability_revision;
...
const Conversation = struct {
    ...
    fn init(self: *Conversation, script: []const u8) !void {
        self.fake = try acp.FakeAgent.init(testing.allocator, script);
        self.adapter = acp.Adapter.init(testing.allocator, self.fake.config());
...
test "an ACP permission turn served through the endpoint completes, and the whole exchange validates" {
    var conversation: Conversation = undefined;
    try conversation.init(acp.fake_prelude ++ acp.fake_permission_turn ++ acp.fake_idle);
    ...
    const resolved = try conversation.approve(gate, "allow");
...
test "an ACP run cancelled through the endpoint is acknowledged before it settles cancelled, and the exchange validates" {
    var conversation: Conversation = undefined;
    try conversation.init(acp.fake_prelude ++ acp.fake_cancelled_turn ++ acp.fake_idle);
    defer conversation.deinit();
    try conversation.open();
...
test "an ACP agent that exits mid-run fails the run it held, and the exchange validates" {
    var conversation: Conversation = undefined;
    try conversation.init(acp.fake_prelude ++ "take\nexit 7\n");
    defer conversation.deinit();
    try conversation.open();
