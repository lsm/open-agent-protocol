...
const opencode = @import("opencode_adapter");
...
const revision = opencode.capability_revision;
const session_id = "e2e";

const Conversation = struct {
    fake: opencode.FakeServer,
    ...
    fn init(self: *Conversation, turns: []const []const []const u8, after_interrupt: []const []const u8) !void {
        self.fake = .{ .turns = turns, .after_interrupt = after_interrupt };
        try self.fake.start();
        self.arena = std.heap.ArenaAllocator.init(testing.allocator);
        self.adapter = opencode.Adapter.init(testing.allocator, try self.fake.config(self.arena.allocator()));
...
test "an OpenCode turn served through the endpoint completes, a queued one follows it, and the whole exchange validates" {
    var conversation: Conversation = undefined;
    try conversation.init(&.{ &opencode.text_turn, &opencode.text_turn }, &.{});
    ...
    try testing.expectEqualStrings("session.message.submit.response", typeOf(queued[0]));
    _ = try conversation.pumpUntil("run.completed");
