...
const hermes = @import("hermes_adapter");
...
const revision = hermes.capability_revision;
const session_id = "e2e";

const Conversation = struct {
    fake: hermes.FakeGateway,
    ...
    fn init(self: *Conversation, script: []const u8) !void {
        self.fake = try hermes.FakeGateway.init(testing.allocator, script);
        self.adapter = hermes.Adapter.init(testing.allocator, self.fake.config());
    ...
    fn answer(self: *Conversation, gate: std.json.Value, question: []const u8, choice: []const u8) ![]std.json.Value {
        ...
        const payload = try std.fmt.allocPrint(scratch,
            \\{{"interaction_id":"{s}","requested_by":"{s}","responded_by":"{s}","session_id":"{s}","run_id":"{s}","answers":[{{"question_id":"{s}","selected_option_ids":["{s}"]}}]}}
        , .{ asked.get("interaction_id").?.string, asked.get("requested_by").?.string, asked.get("responded_by").?.string, session_id, run_id, question, choice });
...
test "a Hermes approval turn served through the endpoint completes, and the whole exchange validates" {
    var conversation: Conversation = undefined;
    try conversation.init(hermes.fake_prelude ++ hermes.fake_approval_turn ++ hermes.fake_idle);
    ...
    const resolved = try conversation.answer(gate, "choice", "once");
    try testing.expectEqualStrings("user.input.resolve.response", typeOf(resolved[0]));
    const settled = for (resolved) |written| {
        if (std.mem.eql(u8, typeOf(written), "run.completed")) break true;
    } else false;
    if (!settled) _ = try conversation.pumpUntil("run.completed");
...
test "a Hermes run cancelled through the endpoint is acknowledged before it settles, and the exchange validates" {
    var conversation: Conversation = undefined;
    try conversation.init(hermes.fake_prelude ++ hermes.fake_interrupted_turn ++ hermes.fake_idle);
    defer conversation.deinit();
    try conversation.open();
    ...
    try testing.expectEqualStrings("hermes_interrupted", errorCode(failed));
