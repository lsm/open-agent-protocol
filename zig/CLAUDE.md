...
- Release: tagging `v*` runs `.github/workflows/release-binaries.yml` (six targets) and `scripts/package-npm.ts` builds the `@oap-sdk/cli-<platform>` packages, one per platform, each shipping the `oapx` binary for that target. Nothing dispatches between them yet: the launcher that did belonged to the SDK package, which no longer carries a `bin`, so a CLI package of its own has still to be written. CI's cross-compile smoke job builds the non-native targets on every PR.
