...
const unported_backends = [_][]const u8{ "deepseek", "memory", "opencode" };
...
fn runBackendMode(
    ...
    const served = if (std.mem.eql(u8, entry.kind, "claude")) claude_served: {
        ...
        try writeBackendRefusal(stderr, arena, "backend \"{s}\" is of type \"{s}\", which oapx does not know; it serves claude, codex, pi, acp and hermes", .{ name, entry.kind });
        return error.BackendRefused;
    };
...
test "a backend oapx does not know, or a --config entry it cannot serve, is refused on stderr before any request is read" {
    ...
    try std.testing.expectEqualStrings("oapx serve agent: backend \"nonesuch\" is of type \"nonesuch\", which oapx does not know; it serves claude, codex, pi, acp and hermes\n", unknown);
