...
entry an ACP v1 agent, a `hermes` entry a Hermes gateway (pinned `v2026.9.24`), a `deepseek` entry a DeepSeek harness (pinned `47f9438`), and an `opencode` entry an OpenCode server (pinned `v1.18.29`), behind the same stdio door instead of the built-in loop, following
