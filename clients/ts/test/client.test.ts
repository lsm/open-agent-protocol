...
test('dial accepts a bare address and lists adapters', async () => {
  const transport = new FakeTransport([
    {
      ...
      body: (call) => JSON.stringify({
        adapters: [
          {
            name: 'memory',
            capability_revision: 'reference-memory-v8',
  ...
  assert.equal(adapters[0].capability_revision, 'reference-memory-v8');
...
test('capabilities returns the revision and descriptor', async () => {
  const transport = new FakeTransport([
    {
      ...
      body: (call) => JSON.stringify(
        testEnvelope({
          ...
          capabilityRevision: 'reference-memory-v8',
  ...
  assert.equal(caps.revision, 'reference-memory-v8');
...
function catalogTransport(payload: Record<string, unknown>, revision = 'reference-memory-v8'): FakeTransport {
...
test('a catalog payload naming this session is accepted', async () => {
  ...
  assert.equal(catalog.revision, 'reference-memory-v8');
});
