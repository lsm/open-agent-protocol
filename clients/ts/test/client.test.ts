...
test('dial accepts a bare address and lists adapters', async () => {
  ...
  assert.equal(adapters[0].capability_revision, 'reference-memory-v11');
  ...
  const transport = new FakeTransport([
    {
      ...
      body: (call) => JSON.stringify(
        testEnvelope({
          ...
          capabilityRevision: 'reference-memory-v11',
...
function catalogTransport(payload: Record<string, unknown>, revision = 'reference-memory-v11'): FakeTransport {
...
test('a catalog payload naming this session is accepted', async () => {
  ...
  assert.equal(catalog.revision, 'reference-memory-v11');
});
