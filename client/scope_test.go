...
func stateResponse(t *testing.T, envelopeScope, payloadScope protocol.SessionID) protocol.Envelope {
	...
	response.SessionID, response.InReplyTo, response.CapabilityRevision = envelopeScope, "someone-elses-request", "reference-memory-v6"
	return response
}
...
func TestClientRejectsACatalogItCannotBindToADescriptor(t *testing.T) {
	...
	if listing.Revision != "reference-memory-v6" {
