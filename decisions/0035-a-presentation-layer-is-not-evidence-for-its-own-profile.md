...
- The gap a step-1-and-2 change will have to close, and which a schema alone
  ...
  `examples/presentation-control-session.json` stays the profile's only worked
  instance until a producer regenerates it, and a schema that makes it fail is
  a sign the schema, not the example, is wrong.
