...
You also need access to the Makai runtime binary. By default the SDK prefers the installed `@oap-sdk/cli-<platform>-<arch>` optional dependency, then a local build under `zig-out/bin/makai` or `zig/zig-out/bin/makai`, then `makai` on `PATH`. See [Configuration](#configuration) for explicit binary resolver options.
