...
func (a *Adapter) Probe(ctx context.Context) (base.Descriptor, error) {
	...
	features := map[string]protocol.FeatureSupport{
		...
		"session.message.delivery.queue": {Level: protocol.SupportDegraded, Reason: "queue is a native server delivery, but OAP v0.1 canonical admission resolves every submission to one started run; exposing it needs an OAP admission extension"},
		"session.message.delivery.steer": {Level: protocol.SupportDegraded, Reason: "steer is the server default delivery, but OAP v0.1 canonical admission resolves every submission to one started run; exposing it needs an OAP admission extension"},
