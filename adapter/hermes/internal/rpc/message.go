...
var (
	ErrInvalidMessage = errors.New("hermes rpc: invalid JSON-RPC message")
...
func (id *RequestID) UnmarshalJSON(data []byte) error {
	...
	if err := ensureEOF(decoder); err != nil {
		return fmt.Errorf("%w: trailing JSON: %v", ErrInvalidID, err)
	}
...
func ParseMessage(data []byte) (Message, error) {
	...
	if err := ensureEOF(decoder); err != nil {
		return Message{}, fmt.Errorf("%w: trailing JSON: %v", ErrInvalidMessage, err)
	}
	allowed := map[string]bool{"jsonrpc": true, "id": true, "method": true, "params": true, "result": true, "error": true}
	...
	if hasError {
		...
		if err := decodeStrictObject(object["error"], &fields); err != nil {
			return Message{}, fmt.Errorf("%w: malformed error object: %v", ErrInvalidMessage, err)
		}
		for name := range fields {
			if name != "code" && name != "message" && name != "data" {
				return Message{}, fmt.Errorf("%w: unknown error member %q", ErrInvalidMessage, name)
			}
		}
		...
		if err := decodeStrictObject(object["error"], &rpcError); err != nil || rpcError.Message == "" {
...
func (message Message) MarshalJSON() ([]byte, error) {
	...
	switch message.Kind {
	case MessageRequest:
		if !message.ID.valid() || message.Method == "" || (len(message.Params) > 0 && !rawObjectOrNull(message.Params)) {
	...
	case MessageNotification:
		if message.Method == "" || (len(message.Params) > 0 && !rawObjectOrNull(message.Params)) {
			return nil, ErrInvalidMessage
		}
		object["method"] = message.Method
		putRaw(object, "params", message.Params)
...
func decodeStrictObject(data []byte, dst any) error {
	decoder := json.NewDecoder(bytes.NewReader(data))
...
func rawObjectOrNull(raw json.RawMessage) bool {
	if len(raw) == 0 {
		return false
	}
